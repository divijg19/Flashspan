...
export async function setColorScheme(
	...
	return invoke<AppSettings>("set_color_scheme", {
...
// --- Events (typed) ---
export function onCountdownTick(
	handler: (value: string) => void,
): Promise<UnlistenFn> {
	return listen<string>("countdown_tick", (event) =>
		handler(String(event.payload ?? "")),
	);
}

export function onShowNumber(
	handler: (payload: ShowNumber) => void,
): Promise<UnlistenFn> {
	return listen<ShowNumber>("show_number", (event) => handler(event.payload));
}

export function onClearScreen(
...
export function onAutoRepeatTick(
	...
	return listen<AutoRepeatTickPayload>("auto_repeat_tick", (event) =>
		handler(event.payload),
	);
}
...
export function onAppSettingsChanged(
	...
	return listen<AppSettings>("app_settings_changed", (event) =>
		handler(event.payload),
	);
}
...
export function onSessionComplete(
	...
	return listen<SessionComplete>("session_complete", (event) =>
		handler(event.payload),
	);
}
